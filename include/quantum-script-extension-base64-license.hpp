//
// Quantum Script Extension Base64
//
// Copyright (c) 2020 Grigore Stefan <g_stefan@yahoo.com>
// Created by Grigore Stefan <g_stefan@yahoo.com>
//
// MIT License (MIT) <http://opensource.org/licenses/MIT>
//

#ifndef QUANTUM_SCRIPT_EXTENSION_BASE64_LICENSE_HPP
#define QUANTUM_SCRIPT_EXTENSION_BASE64_LICENSE_HPP

#ifndef QUANTUM_SCRIPT_EXTENSION_BASE64__EXPORT_HPP
#include "quantum-script-extension-base64--export.hpp"
#endif

namespace Quantum {
	namespace Script {
		namespace Extension {
			namespace Base64 {
				namespace License {
					QUANTUM_SCRIPT_EXTENSION_BASE64_EXPORT const char *content();
					QUANTUM_SCRIPT_EXTENSION_BASE64_EXPORT const char *shortContent();
				};
			};
		};
	};
};

#endif
